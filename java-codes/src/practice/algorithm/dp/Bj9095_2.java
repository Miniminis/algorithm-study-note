...
public class Bj9095_2 {
    ...
    private static int tripleFib(int n) {
        ...
        if (n == 1) {
            return initialization(1, 1);
        }

        if (n == 2) {
            return initialization(2, 2);
        }

        if (n == 3) {
            return initialization(3, 4);
        }

        cache[n] = tripleFib(n-1) + tripleFib(n-2) + tripleFib(n-3);
        return cache[n];
    }

    public static int initialization(int n, int v) {
        cache[n] = v;
        return cache[n];
    }
